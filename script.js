...
function saveCart() {
  try {
    localStorage.setItem('bgwCart', JSON.stringify(cart));
  } catch (error) {
...
function loadCart() {
  try {
    const stored = localStorage.getItem('bgwCart');
    if (!stored) return;
    const parsed = JSON.parse(stored);
    if (!Array.isArray(parsed)) return;
    cart.length = 0;
    parsed.forEach(item => {
      if (item && item.name && Number(item.price) >= 0 && Number(item.quantity) > 0) {
        cart.push({
          name: item.name,
...
function clearCart() {
  if (cart.length === 0) {
    toastAlert('Your cart is already empty.');
    return;
  }
  ...
  renderCart();
  renderCartSidebar();
  toastAlert('Cart has been cleared.');
}

function checkoutCart() {
  if (cart.length === 0) {
    toastAlert('Your cart is empty. Add wine before checkout.');
    return;
  }

  const grandTotal = cart.reduce((acc, item) => acc + item.price * item.quantity, 0).toFixed(2);
  const confirmed = confirm(`Checkout total: ₾${grandTotal}. Would you like to pay now?`);

  if (confirmed) {
    clearCart();
    toastAlert(`Thank you! Payment completed for ₾${grandTotal}. Your order is confirmed.`);
  }
}
...
function renderCartSidebar() {
  ...
  if (cart.length === 0) {
    ...
    const existingCheckout = document.getElementById('sidebarCheckoutBtn');
  ...
  let checkoutBtn = document.getElementById('sidebarCheckoutBtn');
  if (!checkoutBtn) {
    checkoutBtn = document.createElement('button');
    checkoutBtn.id = 'sidebarCheckoutBtn';
    checkoutBtn.className = 'btn';
    checkoutBtn.style.marginTop = '0.75rem';
